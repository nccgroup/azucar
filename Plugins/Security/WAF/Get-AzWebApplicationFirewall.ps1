...
[cmdletbinding()]
    ...
    Process{
        ...
        Write-AzucarMessage -WriteLog $WriteLog -Message ("Searching for Web Application Gateways in {0}..." -f $Instance.ResourceManager) `
    ...
    End{
        if($AllWAFInfo){
            ...
            if($AllWAF){
                $ReturnPluginObject | Add-Member -type NoteProperty -name WebApplicationFirewall -value $AllWAF
            }
        }
        else{
