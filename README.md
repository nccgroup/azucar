...
**To use your custom plugin**, add it to the Plugins\Custom folder. For those not familiar with plugins, it's essentially any valid PowerShell script saved with a .ps1 extension. Each is self-contained PowerShell code that will be passed as a scriptblock class. The variable names and return values are the same throughout all plugins, so they can be generically loaded. 
